...
const STATUS_CONFIG: Record<LogStatus, { labelKey: string; cls: string; icon: React.ElementType }> = {
  Success:  { labelKey: "sec.statusSuccess",  cls: "bg-emerald-100 text-emerald-700 border-emerald-200",  icon: CheckCircle2   },
  Verified: { labelKey: "sec.statusVerified", cls: "bg-sky-100     text-sky-700     border-sky-200",      icon: ShieldCheck    },
  Warning:  { labelKey: "sec.statusWarning",  cls: "bg-amber-100   text-amber-700   border-amber-200",    icon: AlertTriangle  },
  Failed:   { labelKey: "sec.statusFailed",   cls: "bg-rose-100    text-rose-700    border-rose-200",     icon: XCircle        },
  Blocked:  { labelKey: "sec.statusBlocked",  cls: "bg-red-100     text-red-700     border-red-200 animate-pulse", icon: AlertCircle },
};

const CATEGORY_CONFIG: Record<LogCategory, { labelKey: string; icon: React.ElementType; color: string }> = {
  auth:    { labelKey: "sec.catAuth",    icon: LogIn,       color: "text-sky-600"     },
  config:  { labelKey: "sec.catConfig",  icon: Settings,    color: "text-violet-600"  },
  access:  { labelKey: "sec.catAccess",  icon: Key,         color: "text-primary"     },
  lockout: { labelKey: "sec.catLockout", icon: Lock,        color: "text-red-600"     },
  system:  { labelKey: "sec.catSystem",  icon: ShieldCheck, color: "text-emerald-600" },
};
...
export default function SecurityManagement() {
  ...
  useEffect(() => {
    ...
    // Log page visit
    AuditLogger.access(t("sec.logAccess"), "Success", t("sec.logAccessDesc"));
  ...
  const handleSavePin = async () => {
    if (newPin.length !== 6 || isNaN(Number(newPin))) {
      toast({ title: t("sec.toastInvalidPin"), description: t("sec.toastInvalidPinDesc"), variant: "destructive" });
      return;
    }
    if (newPin !== confirmPin) {
      toast({ title: t("sec.toastPinMismatch"), description: t("sec.toastPinMismatchDesc"), variant: "destructive" });
      return;
    }
    setIsSavingPin(true);
    await setDoc(doc(db, "settings", "global"), { masterPin: newPin }, { merge: true });
    await AuditLogger.auth(t("sec.logPinRotated"), "Success", t("sec.logPinRotatedDesc"));
    setNewPin(""); setConfirmPin("");
    setTimeout(() => {
      setIsSavingPin(false);
      toast({ title: t("sec.toastPinUpdated"), description: t("sec.toastPinUpdatedDesc") });
    }, 800);
  };
  ...
  const verifyPinChallenge = async (): Promise<boolean> => {
    if (pinChallenge.length !== 6) {
      setChallengeError(t("sec.challengeTitle"));
      return false;
    }
    ...
    setChallengeError(t("sec.challengeIncorrect"));
    await AuditLogger.blocked(t("sec.logPanicAuthFailed"), t("sec.logPanicAuthFailedDesc"));
    return false;
  };
  ...
  const handleInitiateLock = async () => {
    ...
    await AuditLogger.lockout(
      t("sec.logPanicInitiated"),
      "Blocked",
      t("sec.logPanicInitiatedDesc")
    );
    setShowLockDialog(false); setPinChallenge(""); setIsProcessingLock(false);
    toast({
      variant: "destructive",
      title: t("sec.toastLockoutActive"),
      description: t("sec.toastLockoutActiveDesc"),
    });
  };
  ...
  const handleTerminateLock = async () => {
    ...
    await AuditLogger.lockout(
      t("sec.logPanicTerminated"),
      "Verified",
      t("sec.logPanicTerminatedDesc")
    );
    setShowUnlockDialog(false); setPinChallenge(""); setIsProcessingLock(false);
    toast({ title: t("sec.toastSystemRestored"), description: t("sec.toastSystemRestoredDesc") });
  };
  ...
  const handleExportLogs = () => {
    ...
    AuditLogger.system(t("sec.logExported"), "Success", t("sec.logExportedDesc").replace("{count}", String(allLogs.length)));
  };
  ...
  const PinChallengeForm = ({
    ...
    <div className="space-y-5 pt-2">
      ...
      >
        ...
        {isProcessingLock ? t("sec.verifying") : title}
      </Button>
    </div>
  );
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
        ...
        <div className="lg:col-span-3 space-y-6">
          ...
          <Card className="border-primary/10 shadow-sm overflow-hidden rounded-2xl">
            <CardHeader className="bg-muted/30 border-b py-5 px-6">
              <CardTitle className="text-base font-bold flex items-center gap-2">
                <Key className="w-5 h-5 text-accent" /> {t("sec.masterPin")}
              </CardTitle>
              <CardDescription>
                {t("sec.masterPinDesc")}
              </CardDescription>
            ...
            <CardContent className="pt-6 px-6 space-y-5">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  <Label htmlFor="new-pin">{t("sec.newPin")}</Label>
                  <div className="relative">
                    <Input
                      id="new-pin"
                      type={showPin ? "text" : "password"}
                ...
                <div className="space-y-2">
                  <Label htmlFor="confirm-pin">{t("sec.confirmPin")}</Label>
                  <Input
                    id="confirm-pin"
                    type={showPin ? "text" : "password"}
                    value={confirmPin}
              ...
              {confirmPin && confirmPin !== newPin && (
                <p className="text-xs text-red-500 flex items-center gap-1.5">
                  <AlertCircle className="w-3.5 h-3.5" /> {t("sec.pinMismatch")}
                </p>
              )}
              <p className="text-[10px] text-muted-foreground italic">
                {t("sec.pinRotationAdvice")}
              </p>
            </CardContent>
          ...
          <Card className="border-primary/10 shadow-sm overflow-hidden rounded-2xl">
            <CardHeader className="bg-muted/30 border-b py-5 px-6">
              <CardTitle className="text-base font-bold flex items-center gap-2">
                <Lock className="w-5 h-5 text-accent" /> {t("sec.hardeningTitle")}
              </CardTitle>
              <CardDescription>
                {t("sec.hardeningDesc")}
              </CardDescription>
              ...
              {[
                {
                  ...
                  onChange: (v: boolean) => {
                    setTwoFactor(v);
                    AuditLogger.config(v ? t("sec.logMfaEnabled") : t("sec.logMfaDisabled"), v ? "Success" : "Warning");
                    toast({ title: v ? t("sec.toastMfaEnabled") : t("sec.toastMfaDisabled") });
                  },
                  label: t("sec.mfa"),
                  desc:  t("sec.mfaDesc"),
                },
                {
                  ...
                  onChange: (v: boolean) => {
                    setHijackProtect(v);
                    AuditLogger.config(v ? t("sec.logHijackEnabled") : t("sec.logHijackDisabled"), v ? "Success" : "Warning");
                    toast({ title: t("sec.toastSessionPolicyUpdated") });
                  },
                  label: t("sec.hijack"),
                  desc:  t("sec.hijackDesc"),
                },
                {
                  key: "rotate",
                  checked: logRotation,
                  onChange: (v: boolean) => {
                    setLogRotation(v);
                    AuditLogger.config(v ? t("sec.logRotationEnabled") : t("sec.logRotationDisabled"), v ? "Success" : "Warning");
                  },
                  label: t("sec.logRotate"),
                  desc:  t("sec.logRotateDesc"),
                },
              ].map((item, idx, arr) => (
                <div key={item.key}>
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className={cn(
            ...
            <div className="flex gap-3 items-start mb-4">
              ...
              <div>
                <p className={cn("text-sm font-black leading-tight", isLockedOut ? "text-red-900" : "text-slate-800")}>
                  {isLockedOut ? `⚠️ ${t("sec.panicTitle")}` : t("sec.panicTitle")}
                </p>
                <p className={cn("text-[11px] leading-snug mt-1.5", isLockedOut ? "text-red-700" : "text-muted-foreground")}>
                  {isLockedOut
                    ? t("sec.panicActiveDesc")
                    : t("sec.panicInactiveDesc")
                  }
                </p>
              </div>
            </div>
            ...
            {isLockedOut ? (
              ...
              >
                <Unlock className="w-4 h-4" /> {t("sec.panicRestore")}
              </Button>
            ) : (
              <Button
                variant="destructive"
              ...
              >
                <ShieldAlert className="w-4 h-4" /> {t("sec.panicInitiate")}
              </Button>
            )}
          </div>
            ...
            <CardHeader className="bg-muted/30 border-b py-4 px-5 flex flex-row items-center justify-between gap-3">
              <div className="flex items-center gap-2">
                ...
                <Badge variant="outline" className="text-[8px] font-black bg-white px-2">
                  {t("sec.logLast").replace("{limit}", String(LOG_PREVIEW_LIMIT))}
                </Badge>
              </div>
              <div className="flex items-center gap-1">
                ...
                >
                  <Archive className="w-3 h-3" /> {t("sec.archive")}
                </Button>
              </div>
            </CardHeader>
            <CardContent className="pt-2 px-3 pb-3">
              {previewLogs.length === 0 ? (
                <p className="text-[10px] text-muted-foreground text-center py-6 italic">{t("sec.noLogs")}</p>
              ) : (
            ...
            {allLogs.length > LOG_PREVIEW_LIMIT && (
              <CardFooter className="border-t py-3 px-5 bg-muted/10">
                <Button
                  ...
                  onClick={() => setIsArchiveOpen(true)}
                >
                  {t("sec.viewAllCount").replace("{count}", String(allLogs.length))} →
                </Button>
              </CardFooter>
            )}
          </Card>
        ...
        <DialogContent className="sm:max-w-md rounded-2xl border-red-200">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2 text-red-700">
              <ShieldAlert className="w-5 h-5" /> {t("sec.confirmTitle")}
            </DialogTitle>
            <DialogDescription>
              {t("sec.confirmDesc")}
            </DialogDescription>
          ...
          <PinChallengeForm
            title={t("sec.confirmAction")}
            subtitle={t("sec.confirmSubtitle")}
        ...
        <DialogContent className="sm:max-w-md rounded-2xl border-emerald-200">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2 text-emerald-700">
              <Unlock className="w-5 h-5" /> {t("sec.panicRestore")}
            </DialogTitle>
            <DialogDescription>
              {t("sec.restoreDesc")}
            </DialogDescription>
          ...
          <PinChallengeForm
            title={t("sec.restoreAction")}
            subtitle={t("sec.restoreSubtitle")}
        ...
        <DialogContent className="sm:max-w-3xl rounded-2xl">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              ...
              {t("sec.archiveTitle")}
              <Badge variant="outline" className="ml-2 text-[9px] font-black">{filteredArchiveLogs.length} {t("sec.entries")}</Badge>
            </DialogTitle>
            <DialogDescription>{t("sec.archiveDesc")}</DialogDescription>
          </DialogHeader>
          ...
          <div className="flex flex-wrap gap-3 mt-1">
            ...
            <Select value={filterStatus} onValueChange={setFilterStatus}>
              <SelectTrigger className="h-9 w-36 rounded-xl text-xs">
                ...
                <SelectValue placeholder={t("sec.status")} />
              </SelectTrigger>
              <SelectContent className="rounded-xl">
                <SelectItem value="all">{t("sec.allStatus")}</SelectItem>
                {(["Success","Verified","Warning","Failed","Blocked"] as LogStatus[]).map(s => (
                  <SelectItem key={s} value={s}>{t(STATUS_CONFIG[s].labelKey as any)}</SelectItem>
                ))}
              </SelectContent>
            </Select>
            <Select value={filterCategory} onValueChange={setFilterCategory}>
              <SelectTrigger className="h-9 w-40 rounded-xl text-xs">
                <SelectValue placeholder={t("sec.category")} />
              </SelectTrigger>
              <SelectContent className="rounded-xl">
                <SelectItem value="all">{t("sec.allCategory")}</SelectItem>
                {(["auth","config","access","lockout","system"] as LogCategory[]).map(c => (
                  <SelectItem key={c} value={c}>{t(CATEGORY_CONFIG[c].labelKey as any)}</SelectItem>
                ))}
              </SelectContent>
            </Select>
            <Button variant="outline" size="sm" className="h-9 rounded-xl text-xs gap-1.5" onClick={handleExportLogs}>
              <Download className="w-3.5 h-3.5" /> {t("sec.export")}
            </Button>
          </div>
          ...
          <ScrollArea className="h-[440px] mt-2 rounded-xl border bg-muted/10 p-3">
            <div className="space-y-2">
              {filteredArchiveLogs.length === 0 ? (
                <p className="text-xs text-muted-foreground text-center py-10 italic">{t("sec.noMatches")}</p>
              ) : (
            ...
            {(["Success","Verified","Warning","Failed","Blocked"] as LogStatus[]).map(s => {
              ...
              return (
                <div key={s} className={cn("rounded-xl border text-center py-2 px-1", sc.cls)}>
                  ...
                  <p className="text-[8px] font-black uppercase tracking-widest opacity-70">{t(sc.labelKey as any)}</p>
                </div>
              );
            })}
          </div>
